...
if DOCKER_SOCKET="$(detect_docker)"; then
  ...
  if [ "${COMPILE_MODE}" = "docker" ]; then
    log_fatal "compile_mode=docker but no Docker socket found. Set 'docker_api': true in config.json and restart."
  else
...
if [ -n "${DOCKER_SOCKET}" ] && [ "${COMPILE_MODE}" != "builtin" ]; then
  if ! command -v docker >/dev/null 2>&1; then
    ...
    DOCKER_SOCKET=""
  elif ! docker inspect "${ESPHOME_CONTAINER}" >/dev/null 2>&1; then
...
# Built-in compiler path (venv)
if [ -z "${DOCKER_SOCKET}" ] || [ "${COMPILE_MODE}" = "builtin" ]; then
  VENV_DIR="/data/venv"
  ESPHOME_BIN="${VENV_DIR}/bin/esphome"

  if [ ! -x "${ESPHOME_BIN}" ]; then
    log_info "Bootstrapping built-in ESPHome venv at ${VENV_DIR} (first run only)..."
    # Use virtualenv so pip is present inside the venv on Alpine
